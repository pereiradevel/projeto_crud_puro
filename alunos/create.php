...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($matricula) && !empty($nome_aluno) && !empty($turma_id)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO alunos (matricula, nome_aluno, turma_id) VALUES (:matricula, :nome_aluno, :turma_id)");
            $stmt->execute([
                ':matricula' => $matricula,
    ...
    <div class="max-w-2xl mx-auto px-4 py-10">
        ...
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
                <i class='bx bx-user-plus text-indigo-600'></i> Cadastrar Aluno
            </h1>
            <p class="text-sm text-slate-500 mt-1">Adicione um novo estudante preenchendo as informações abaixo.</p>
        </div>
            ...
            <form action="create.php" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="matricula" name="matricula" required placeholder="Ex: 2026001"
                            class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-mono focus:outline-none focus:border-indigo-500 focus:bg-white transition-all text-sm text-slate-900 placeholder:text-slate-400">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="nome_aluno" name="nome_aluno" required placeholder="Ex: Carlos Henrique Silva"
                            class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 focus:bg-white transition-all text-sm text-slate-900 placeholder:text-slate-400">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select id="turma_id" name="turma_id" required 
                            ...
                            <?php
                            $stmt_turmas = $pdo->query("SELECT id, nome_turma FROM turmas ORDER BY nome_turma ASC");
                ...
                <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-100">
                    ...
                    <button type="submit" class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-6 py-2.5 rounded-xl shadow-md shadow-indigo-100 transition-all text-sm flex items-center justify-center gap-2">
                        <i class='bx bx-check text-xl'></i> Salvar Aluno
                    </button>
                </div>
