...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($id) && !empty($nome) && !empty($cargo)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO funcionarios (registro_funcional, nome, cargo) VALUES (:registro_funcional, :nome, :cargo)");
            $stmt->execute([
                ':registro_funcional' => $id,
                ':nome' => $nome,
                ':cargo' => $cargo
            ]);
...
<head>
    ...
    <title>Novo Funcionário - SADE</title>
    ...
    <div class="max-w-2xl mx-auto px-4 py-10">
        ...
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
                <i class='bx bx-user-plus text-amber-600'></i> Cadastrar Funcionário
            </h1>
            <p class="text-sm text-slate-500 mt-1">Adicione um novo membro à equipe escolar preenchendo as informações abaixo.</p>
        </div>
            ...
            <form action="create.php" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="id" name="id" required placeholder="Ex: F-102"
                            class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-mono focus:outline-none focus:border-amber-500 focus:bg-white transition-all text-sm text-slate-900 placeholder:text-slate-400">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="nome" name="nome" required placeholder="Ex: Mariana Costa Ramos"
                            class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-amber-500 focus:bg-white transition-all text-sm text-slate-900 placeholder:text-slate-400">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="cargo" name="cargo" required placeholder="Ex: Coordenadora Pedagógica"
                            class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-amber-500 focus:bg-white transition-all text-sm text-slate-900 placeholder:text-slate-400">
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-100">
                    ...
                    <button type="submit" class="w-full sm:w-auto bg-amber-600 hover:bg-amber-700 text-white font-semibold px-6 py-2.5 rounded-xl shadow-md shadow-amber-100 transition-all text-sm flex items-center justify-center gap-2">
                        <i class='bx bx-check text-xl'></i> Salvar Funcionário
                    </button>
                </div>
